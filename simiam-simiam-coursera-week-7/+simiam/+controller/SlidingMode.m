    ...
    methods
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(~obj.calibrated)
                ...
                obj.v_gtg = plot(robot.parent, [0;0],[0;0],'b-', 'LineWidth', 2);
                obj.v_ao = plot(robot.parent, [0;0],[0;0],'m-', 'LineWidth', 2);                
                obj.v_ufw = plot(robot.parent, [0;0],[0;0],'c-', 'LineWidth', 2);
                set(obj.v_gtg, 'ZData', [2;2]);
                set(obj.v_ao, 'ZData', [2;2]);
                set(obj.v_ufw, 'ZData', [2;2]);
            end
...
%             fprintf('obj.u_gtg %0.3f %0.3f\n', obj.u_gtg);
            % 2. Compute u_ao
%             sensor_gains = [1 1 0.5 1 1];
%             sensor_gains = [2 1 1 1 2];
            sensor_gains = ir_distances;
...
%              set(obj.v_gtg, 'XData', [u_fw_t(1)+p_1(1);p_1(1)]);%blue
%             set(obj.v_gtg, 'YData', [u_fw_t(2)+p_1(2);p_1(2)]);
%             set(obj.v_ao, 'XData', [obj.u_ao(1)+x;x]);%magenta
%             set(obj.v_ao, 'YData', [obj.u_ao(2)+y;y]);
%             set(obj.v_ufw, 'XData', [obj.u_fw(1)+x;x]);%cyan
%             set(obj.v_ufw, 'YData', [obj.u_fw(2)+y;y]);
