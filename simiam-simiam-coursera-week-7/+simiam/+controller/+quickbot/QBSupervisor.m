...
%             obj.goal        = [0, -1.1];
%             obj.goal        = [1.1, -1.1];
%             obj.goal        = [1.1, 0]; %2circles theta=0
%             obj.goal        = [1.1, 1.1];
%             obj.goal        = [0, 1.1];
%             obj.goal        = [-1.1, 1.1]; %oscillations in the end with theta=0
            obj.goal        = [-1.1, 0];
            obj.d_stop      = 0.05;
            obj.d_at_obs    = 0.29935;      %0.29936          
            obj.d_unsafe    = 0.05;
        ...
        %   See also controller/execute
            ...
            inputs.d_at_obs = obj.d_at_obs;
            [x,y,theta] = obj.state_estimate.unpack();            
            
            %goal marker
            hold(obj.robot.parent, 'on');
            obj.goal_marker = plot(inputs.x_g,inputs.y_g,'r*');
            
            %% START CODE BLOCK %%
            
            ir_distances = obj.robot.get_ir_distances();
%             fprintf('obj.fw_direction %s\n', obj.fw_direction);
            if (obj.check_event('at_goal'))
                if (~obj.is_in_state('stop'))                    
                    fprintf('stopped at (%0.3f,%0.3f)\n', x, y);
                end
                obj.switch_to_state('stop');
            elseif (obj.check_event('unsafe') && ~obj.is_in_state('follow_wall'))
%                 fprintf('checking progress %d\n', strcmp(obj.states{obj.current_state}.state, 'follow_wall'));
                obj.switch_to_state('avoid_obstacles'); 
                obj.d_prog = 10;
            else
                switch obj.states{obj.current_state}.state                    
                    case 'ao_and_gtg'   
%                         fprintf('checking progress\n');
%                         fprintf('current distant %0.10f\n', norm([x-obj.goal(1);y-obj.goal(2)]));
%                         fprintf('progress distant %0.10f\n', obj.d_prog);
                       
                        if (obj.check_event('obstacle_cleared'))
                            obj.switch_to_state('go_to_goal');
                        elseif (~obj.check_event('progress_made'))
                            sides = 0;
                            if (any(ir_distances(1:2) < obj.d_at_obs) && obj.check_event('sliding_left'))
                                obj.fw_direction = 'left';
%                                 fprintf('obj.fw_direction %s\n', obj.fw_direction);
                                sides = sides + 1;
                            end
                            if (any(ir_distances(4:5) < obj.d_at_obs) && obj.check_event('sliding_right'))
...
%                                 fprintf('obj.fw_direction %s\n', obj.fw_direction);
                            ...
                            if (sides > 1)
                                ...
                                cos_u_fw_l = dot(u,obj.u_fw_l)/(norm(u)*norm(obj.u_fw_l));
                                cos_u_fw_r = dot(u,obj.u_fw_r)/(norm(u)*norm(obj.u_fw_r));
%                                 fprintf('cos_u_fw_l %d\n', cos_u_fw_l);
%                                 fprintf('cos_u_fw_r %d\n', cos_u_fw_r);                                
                                if (cos_u_fw_l > cos_u_fw_r)
                                   obj.fw_direction = 'left';
                                else
                                   obj.fw_direction = 'right';
                                end
                    ...
                    case 'follow_wall'
%                         fprintf('current distant %d\n', norm([x-obj.goal(1);y-obj.goal(2)]));
%                         fprintf('progress distant %d\n', obj.d_prog);
                        if (obj.check_event('progress_made'))
%                             fprintf('checking no slide\n');
                            no_slide = false;
%                             fprintf('obj.fw_direction %s\n', obj.fw_direction);
                            if (strcmp(obj.fw_direction,'left'))
%                                 fprintf('checking left no slide\n');
                                no_slide = ~obj.check_event('sliding_left');
                            else
%                                 fprintf('checking right no slide\n');
                                no_slide = ~obj.check_event('sliding_right');
                            end
...
%             fprintf('current_pose: (%0.3f,%0.3f,%0.3f)\n', x, y, theta);
        ...
        function set_progress_point(obj)
            [x, y, theta] = obj.state_estimate.unpack();
%             fprintf('progress distant before %0.3f\n', obj.d_prog);            
            obj.d_prog = min(norm([x-obj.goal(1);y-obj.goal(2)]), obj.d_prog);
%             fprintf('current distant %0.3f\n', norm([x-obj.goal(1);y-obj.goal(2)]));
%             fprintf('progress distance after %0.3f\n', obj.d_prog);
        end
...
%             eps = 0.001;
            eps = 0.02;
            %% START CODE BLOCK %%
%             fprintf('norm([x y] - obj.goal %d\n',norm([x y] - obj.goal));
%             fprintf('obj.d_prog %d\n',obj.d_prog);
%             if (norm([x y] - obj.goal) < obj.d_prog - eps)
             if (norm([x y] - obj.goal) < obj.d_prog + eps)
                rc = true;
%                 fprintf('progress\n');
...
%             if any(ir_distances(2:4) < obj.d_at_obs)
        ...
        function rc = unsafe(obj, state, robot)
            ...
            fprintf('distances\n');
            fprintf('%0.5f\n', ir_distances);
            ir_dist_front = ir_distances(2:4);
            a = any(ir_distances(1:5) < obj.d_unsafe);
            b = ir_distances(3) < obj.d_unsafe + 0.07;            
            c = any(ir_distances([2 4]) < obj.d_unsafe + 0.08);
            d = length(ir_distances(ir_distances(1:5) < obj.d_at_obs - 0.02)) == 1;
            fprintf('%d %d %d %d\n', a, b, c, d);
            % Loop through and test the sensors (only the front set)
            if (a || b || (c && d))
                    rc = true;
...
%             obst_counter = 0;
%             for i=1:length(ir_distances)
%                 if (ir_distances(i) > obj.d_at_obs)
