...
%             sensor_gains = [2 2 0.5 2 2];  %was good for 3 goals
...
%             v = inputs.v*0.1;
            w = obj.Kp*e_P + obj.Ki*e_I + obj.Kd*e_D;
             ...
             % minimal distance to object
            dist = min(norm(u_gtg),min(ir_distances));
            v = abs(w) + dist;            
            if (dist < inputs.d_at_obs)
                v = 0.1; 
%                 fprintf('changing v\n');
...
%             set(obj.v_t, 'XData', [u_ao(1)+x;x]);%red
%             set(obj.v_t, 'YData', [u_ao(2)+y;y]);
%             set(obj.v_p, 'XData', [u_gtg(1)+x;x]);%blue
%             set(obj.v_p, 'YData', [u_gtg(2)+y;y]);
%             set(obj.v_tp, 'XData', [u_ao_gtg(1)+x;x]);%green 
%             set(obj.v_tp, 'YData', [u_ao_gtg(2)+y;y]);
